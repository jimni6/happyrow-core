...
HappyRow est une application web collaborative de gestion d'événements. Elle permet à des groupes de personnes (amis, collègues, famille) d'organiser des événements (fêtes, anniversaires, dîners) et de coordonner les contributions de chaque participant en termes de ressources à apporter (nourriture, boissons, ustensiles, décoration).

Le projet est né du constat que l'organisation d'événements entre plusieurs personnes repose souvent sur des échanges informels (messages, tableurs partagés) qui manquent de structure et de visibilité. HappyRow centralise cette coordination dans une application dédiée.

## 3.2 Objectifs

- Permettre à un utilisateur authentifié de créer et gérer des événements
...
- Assurer la cohérence des données en cas d'accès concurrents (plusieurs participants contribuant simultanément)

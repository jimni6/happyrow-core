...
Pour piloter le projet HappyRow, j'ai adopté une approche **Kanban**, bien adaptée à mon contexte de développeur seul sur un projet de taille moyenne. Concrètement, cela m'a permis de :

- Visualiser mon flux de travail (To Do → In Progress → Done)
- Me limiter à une ou deux tâches en parallèle pour rester concentré
- Livrer de façon continue et incrémentale

J'ai organisé mes tâches sous forme de **tickets** dans un tableau Kanban sur GitHub (Issues + Projects). Chaque ticket correspond à une fonctionnalité, un correctif ou une tâche technique.

## 5.2 Planification et découpage

J'ai découpé le projet en itérations fonctionnelles progressives :
...
J'ai structuré mon workflow Git de la manière suivante :
...
J'ai mis en place un pipeline GitHub Actions dédié au front-end, organisé en 4 jobs :
...
J'ai aussi configuré un workflow **Security Audit** qui s'exécute quotidiennement (cron) : il lance un audit npm, vérifie le lockfile, détecte les scripts d'installation suspects et contrôle la fraîcheur des dépendances.

### Pipeline CI/CD back-end

Pour le back-end, j'ai configuré un pipeline GitHub Actions en 4 étapes :
...
Pour le déploiement, j'ai écrit un **Dockerfile multi-stage** qui sépare bien la phase de build de la phase de production :
...
**Voici comment se déroule un déploiement concrètement :**

1. Je pousse mon code sur la branche `main`
...
4. Render clone le dépôt et exécute le `Dockerfile` multi-stage
...
**Variables d'environnement que j'ai configurées sur Render :**
...
J'ai mis en place plusieurs garde-fous pour maintenir la qualité du code :
...
- J'ai rédigé le code source et la documentation technique en anglais (nommage, commentaires Exposed, Ktor)
- J'ai consulté la documentation technique en anglais : Ktor, Exposed, Arrow, Supabase, OWASP
- Mes commits et pull requests sont en anglais
- La documentation projet (docs/) est rédigée en anglais et en français

...
La fonctionnalité que j'ai choisie comme représentative est **l'ajout d'une contribution avec verrou optimiste**. Elle traverse toutes les couches de l'architecture hexagonale et illustre les choix techniques majeurs que j'ai faits dans ce projet.

---

## Annexe 0 — Maquettes Figma de la fonctionnalité contribution

J'ai réalisé les maquettes ci-dessous sur Figma en amont du développement. Elles utilisent la charte graphique HappyRow (Design Tokens CSS : teal `#5FBDB4`, navy `#3D5A6C`, coral `#E6A19A`, police Comic Neue).
...
J'ai créé ce composant pour organiser les ressources par catégorie. Il itère sur les ressources d'une catégorie donnée et affiche un formulaire inline pour en ajouter de nouvelles.
...
Voici le composant que j'ai développé pour gérer l'interaction de contribution sur chaque ressource. La logique est intelligente : selon la quantité résultante, il décide s'il faut ajouter, mettre à jour ou supprimer la contribution.
...
J'ai implémenté ce repository pour centraliser tous les appels HTTP liés aux contributions. Il utilise l'API native `fetch` avec injection du token JWT via callback.
...
J'ai écrit ce use case pour encapsuler la validation métier avant l'appel au repository :
...
Voici le hook que j'ai développé pour gérer la mise à jour optimiste côté front-end. L'idée est de mettre à jour l'interface immédiatement avant l'appel API, et de revenir en arrière (rollback) si l'appel échoue :

```typescript
// src/features/resources/hooks/useContributionOperations.ts (extrait)
...
J'ai défini cette interface dans le domaine pour que le use case ne dépende pas de l'implémentation SQL :
...
J'ai défini cette table avec toutes ses contraintes (unicité, index, check) directement dans le code Kotlin :
...
J'ai écrit ce mapper pour convertir un `ResultRow` Exposed en objet du domaine, en encapsulant l'opération dans un `Either` :
...
Voici le repository SQL complet que j'ai implémenté. C'est le plus complexe du projet, car il gère l'ajout, la mise à jour, la réduction et la suppression de contributions, avec à chaque fois la mise à jour de la quantité de la ressource associée via le verrou optimiste.
...
C'est la méthode au coeur du mécanisme de verrou optimiste. J'en suis particulièrement fier car elle gère proprement les cas d'erreur (ressource non trouvée vs conflit de version) :
...
Voici le endpoint complet pour l'ajout de contribution, avec la gestion d'erreur spécifique au verrou optimiste :
...
Voici un exemple de test unitaire que j'ai écrit avec la structure BDD (Given/When/Then) et les personas :

...
J'ai organisé mes tests en plusieurs niveaux complémentaires :
...
Voici les outils que j'ai utilisés pour les tests :
...
Voici comment les tests s'intègrent dans mon pipeline :
...
J'ai choisi d'exclure les tests d'intégration (Testcontainers) du pipeline CI via `-PWithoutIntegrationTests` pour des raisons de performance. Je les exécute localement avant chaque mise en production.
...
Pour les tests unitaires front-end, j'utilise `vi.mock()` pour mocker le SDK Supabase et `@testing-library/react` pour tester les composants de manière centrée utilisateur. J'ai concentré mes tests sur le module d'authentification, qui est la fonctionnalité la plus critique.
...
J'ai structuré mes tests selon une approche BDD, avec des fonctions d'aide que j'ai écrites pour la lisibilité :
...
J'ai centralisé mes données de test dans des objets `Persona` partagés entre tous les tests. C'est un choix que j'ai fait pour éviter la duplication et garantir la cohérence des données de test :
...
J'ai choisi **l'ajout d'une contribution avec verrou optimiste** comme fonctionnalité représentative pour le jeu d'essai, car c'est celle qui mobilise le plus de couches :
...
**Analyse des écarts : aucun écart constaté** — le scénario nominal fonctionne exactement comme je l'ai spécifié.

### Scénario d'erreur : conflit de concurrence (verrou optimiste)

Ce scénario est celui qui m'a demandé le plus de réflexion. Il simule deux utilisateurs qui contribuent en même temps :
...
**Analyse des écarts : aucun écart** — le verrou optimiste détecte correctement le conflit et retourne l'erreur appropriée. L'utilisateur B n'a qu'à rafraîchir les données et réessayer.

### Scénario de sécurité : tests de sécurité de la fonctionnalité contribution

J'ai élaboré une série de tests de sécurité pour vérifier que chaque vecteur d'attaque est bien couvert :
...
**Analyse des écarts : aucun écart constaté.** Tous les tests de sécurité confirment que mes mécanismes de protection fonctionnent. La stratégie de défense en profondeur (validation endpoint → validation domaine → contraintes base de données) est effective.

## 10.4 Situation de travail ayant nécessité une recherche

### Contexte : tester un verrou optimiste en conditions de concurrence

Pendant l'implémentation du verrou optimiste sur les contributions, je me suis heurté à une difficulté : **comment tester de manière fiable qu'un accès concurrent est bien détecté et rejeté ?**

### Le problème

Un test unitaire classique (avec MockK) peut simuler le retour d'un `Either.Left(OptimisticLockException)`, mais ça ne prouve pas que le mécanisme fonctionne réellement en base de données avec deux transactions concurrentes. Le risque, c'est qu'en production, le `UPDATE ... WHERE version = N` ne se comporte pas comme attendu à cause de l'isolation transactionnelle ou du verrouillage implicite PostgreSQL.

### Mes recherches

Pour résoudre ce problème, j'ai effectué des recherches sur :

1. **Testcontainers** (documentation officielle + articles Medium) — pour exécuter des tests d'intégration avec une vraie base PostgreSQL dans un conteneur Docker jetable, ce qui garantit un environnement identique à la production.
...
3. **Patterns de test de concurrence en Kotlin** (documentation Kotlin Coroutines) — pour simuler deux opérations concurrentes avec `coroutineScope { launch { ... } }` et vérifier que l'une réussit et l'autre échoue avec `OptimisticLockException`.

### Ce que ça m'a apporté

Ces recherches m'ont permis de :

- Écrire un **test d'intégration avec Testcontainers** qui crée une ressource avec `version=0`, lance deux contributions simultanées avec `expectedVersion=0`, et vérifie que l'une réussit (`Either.Right`) et l'autre échoue (`Either.Left(OptimisticLockException)`)
- Confirmer que le niveau d'isolation `READ COMMITTED` de PostgreSQL est suffisant pour le verrou optimiste (pas besoin de `SERIALIZABLE`)
- Prendre la décision d'exclure ces tests d'intégration du pipeline CI (trop lents) avec le flag `-PWithoutIntegrationTests`, tout en les exécutant localement avant chaque mise en production

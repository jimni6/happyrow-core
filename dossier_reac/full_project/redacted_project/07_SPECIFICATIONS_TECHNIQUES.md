...
La conception de l'application HappyRow intègre des principes d'éco-conception numérique visant à réduire l'empreinte environnementale du service :

| Principe | Application dans HappyRow |
|----------|--------------------------|
| **Sobriété fonctionnelle** | Périmètre fonctionnel limité aux besoins réels (gestion d'événements, participants, ressources, contributions). Pas de fonctionnalités superflues. |
| **Optimisation des requêtes** | L'ORM Exposed génère des requêtes SQL ciblées avec indexes. Le verrou optimiste évite les verrous pessimistes (pas de `SELECT FOR UPDATE`) qui bloqueraient inutilement des lignes. |
| **Minimalisme des dépendances** | Le front-end n'utilise que 3 dépendances de production (React, React Router, Supabase SDK). Pas de framework CSS lourd (CSS natif + Design Tokens). API native `fetch` sans Axios. |
| **Images Docker optimisées** | Build multi-stage : l'image de production ne contient que le JRE 21 et le JAR, pas le SDK Gradle ni les sources. Taille d'image réduite. |
| **PWA et mode offline** | Le service worker (Workbox) met en cache les assets statiques, réduisant les requêtes réseau et la consommation de bande passante lors des visites répétées. |
| **Pas de sur-provisionnement** | JVM tunée (`-Xmx512m -Xms256m`) pour consommer uniquement la mémoire nécessaire. Hébergement Render starter plan dimensionné au besoin réel. |
| **Pool de connexions** | HikariCP mutualise les connexions PostgreSQL au lieu d'en ouvrir/fermer à chaque requête, réduisant la charge serveur et réseau. |

Ces choix contribuent à un usage raisonné des ressources informatiques conformément aux recommandations du référentiel GR491 (Guide de Référence de l'éco-conception de services numériques).
...
Le référentiel CDA (CP8) mentionne les composants d'accès aux données « SQL et NoSQL ». Le projet HappyRow utilise exclusivement **PostgreSQL** (SQL relationnel) pour les raisons suivantes :

| Critère | Justification |
|---------|---------------|
| **Nature des données** | Les données du projet sont fortement relationnelles (événements → participants → ressources → contributions) avec des contraintes d'intégrité référentielle (FK, UNIQUE, CHECK). Un modèle relationnel est le choix le plus adapté. |
| **Intégrité transactionnelle** | Le verrou optimiste et les opérations atomiques (création événement + ajout participant) nécessitent des transactions ACID, point fort des SGBD relationnels. |
| **Complexité du projet** | L'ajout d'une base NoSQL (Redis, MongoDB…) aurait introduit une complexité d'infrastructure non justifiée par les besoins fonctionnels. |
| **PostgreSQL et JSON** | PostgreSQL supporte nativement les types `JSONB` et les arrays (`UUID[]` utilisé pour le champ `members`), ce qui couvre les cas de stockage semi-structuré sans nécessiter un SGBD NoSQL dédié. |

Si le projet devait évoluer (ex : notifications temps réel, analytics, cache de sessions), l'ajout d'un composant NoSQL (Redis pour le cache, ou un stockage documents pour les logs d'événements) serait envisageable dans une version future, comme mentionné dans les perspectives du chapitre 12.

...
L'écran ci-dessus montre la vue détail d'un événement avec ses ressources organisées par catégorie (Food, Drinks). Chaque ressource affiche la quantité courante / suggérée et les contrôles de contribution (+/−). Lorsque l'utilisateur sélectionne une quantité via les boutons +/−, un compteur de delta apparaît et le bouton « Validate » s'affiche pour confirmer la contribution.
